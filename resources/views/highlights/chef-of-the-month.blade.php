    ...
    <div class="mx-auto max-w-5xl space-y-6 px-4 py-8">
        <div class="space-y-1 text-center">
            <div class="flex items-center justify-center gap-2 text-emerald-700">
                ...
                <h1 class="text-2xl font-semibold text-slate-900">{{ __('Chef of the Month') }}</h1>
            </div>
            <p class="text-sm text-slate-600">{{ __('Celebrating the top recipe from') }} {{ $monthLabel }}.</p>
        </div>
        ...
        @if (! $recipe)
            <x-ui.card class="p-6">
                <div class="flex flex-col items-center justify-center gap-3 text-center">
                    ...
                    <p class="text-sm text-slate-600">{{ __('No public recipes were rated last month yet.') }}</p>
                </div>
            </x-ui.card>
        @else
            <x-ui.card class="mx-auto w-full max-w-lg overflow-hidden">
                <div class="flex flex-col gap-6 p-6 md:flex-row md:items-center">
                    <div class="flex items-center gap-4">
                        <div class="flex h-20 w-20 items-center justify-center overflow-hidden rounded-full bg-emerald-100 text-2xl font-semibold text-emerald-700">
                        ...
                        <div>
                            <p class="text-sm text-slate-500">{{ __('Chef of the Month') }}</p>
                            <p class="text-lg font-semibold text-slate-900">{{ $recipe->user?->name ?? __('Unknown chef') }}</p>
                            <p class="text-sm text-slate-600">
                                {{ __('Earned') }} {{ $recipe->stars ?? 0 }} {{ __('stars with') }}
                                <span class="font-medium text-slate-800">{{ $recipe->title }}</span>
                            </p>
                        </div>
                    </div>
            ...
            <x-ui.card class="overflow-hidden">
                <div class="grid gap-6 p-6 md:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
                    ...
                    <div class="space-y-5">
                        <div>
                            <h3 class="text-sm font-semibold text-slate-800">{{ __('Ingredients') }}</h3>
                            <ul class="mt-2 space-y-2 text-sm text-slate-700">
                                ...
                                @empty
                                    <li class="text-sm text-slate-500">{{ __('No ingredients listed.') }}</li>
                                @endforelse
                            </ul>
                        </div>

                        <div>
                            <h3 class="text-sm font-semibold text-slate-800">{{ __('Instructions') }}</h3>
